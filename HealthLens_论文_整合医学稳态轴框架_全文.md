...
**目的**：提出整合医学稳态生物学框架（ISSBF），将古籍的气/血/脏腑/阴阳/正邪/神等概念公理化映射为十条可测量的稳态生物学轴，并建立古籍候选与个体基因弱项通路的融合判定与感知–推断–干预–验证（SIIV）闭环。

**方法**：（1）以六条范式公理约束框架；（2）构建十轴映射；（3）建立真实世界案例证据库（n=29，L1=8，L2=9，L3=12）；（4）设计可穿戴/CGM/BCI数据接口；（5）对融合引擎做in-silico计算验证（22合成画像）。

**结果**：框架在十轴上均能锚定现代机制与人体证据（如断食4周自噬基因LAMP2增加4.2倍；D+Q在240人Phase II RCT中改善步速和握力；16:8限时进食在333人RCT中降低肝脂肪25.8%）。计算验证显示可复现性100%、弱项覆盖率100%、对照画像个性化推荐为0、收敛效度85%。
...
| **I 外泌体胞间通讯** | 气血周流/经络传导 | 外泌体miRNA cargo,CFTR囊泡释放 | 运动后循环外泌体↑2-3倍[19,20]；GeneLLM cfRNA检测[24] |
| **J 再生医学** | 肾精/先天之本 | iPSC重编程,细胞年轻化 | mRNA-LNP递送Oct4/Sox2/Klf4减少衰老细胞(UPenn 2025)；siRNA-RIVER试验HSC年轻化(Phase I)[24] |
...
轴激活分布：A=4, B=5, C=4, D=5, E=3, F=5, G=4, H=3——初始8轴均被激活（I/J轴为后续扩展，需独立验证）。
...
- **vs AI药物发现（C2S-Scale/CellType）**：C2S-Scale用双上下文虚拟筛选做条件推理——药物仅在特定生物背景下有效。ISSBF将同一条件推理方法应用于非药物干预：古籍候选仅在匹配的基因弱项背景下才加权推荐[23]。
- **vs 基因组语言模型（GeneLLM/BioFord）**：GeneLLM将cfRNA原始测序作为语言token处理，无需基因组注释即可发现伪生物标志物。BioFord将实验室变为可编程AI基础设施。ISSBF借鉴此思路：古籍视为"传统医学语言token"，十轴映射为翻译层，SIIV闭环为可学习实验飞轮[24]。

**新颖性声明**：核心增量为（1）古籍→十轴的可计算映射；（2）古籍候选×基因弱项的交集融合；（3）SIIV闭环；（4）硬件数据接口统一定义。不主张超出已知生物学的机制，提供整合与优先级排序的元框架。
...
ISSBF为古今中西结合提供了一套可计算、可验证、去医疗化的理论骨架。以六条公理与十条稳态轴把古籍经验投影为现代可测通路，以29条真实世界案例验证方向正确性，以SIIV闭环实现持续验证。融合引擎计算验证表明该判定可复现、特异、高覆盖且可审计。作为决策支持框架，可成为非用药个性化干预的研究基础，其整体效力有待前瞻性实证。
