"""
agent_safety_gate.py — HealthLens 安全闸门（借鉴 MedAssist 前置红牌 + LabGuard typed guard IR）

设计要点（来自 GOAI 榜单借鉴）：
- MedAssist-Agent（赛道一 #27）：在任何 Agent 运行前先跑规则扫描，命中医学急症即拦截并给急救指引，分级（无/中/高）。
- LabGuard（赛道一 #6 同源）：自然语言规则 → 类型化可执行 IR → 运行时监控器，可量化"不安全事件率"。
...
无第三方依赖，可直接 `python agent_safety_gate.py` 运行演示。
"""
...
_RED_FLAG_PATTERNS = [
    ("胸痛", "可能为心肺急症"),
    ("呼吸困难", "可能为呼吸/心肺急症"),
    ("卒中|中风|半身不遂|口眼歪斜|半边.{0,3}麻木|言语不清|说话含糊", "可能为脑血管意外"),
...
def _make_pattern_finder(patterns):
    ...
    def finder(text: str) -> Optional[str]:
...
def build_rules() -> List[GuardRule]:
    ...
    rules.append(GuardRule(
        id="RF-001", category=GuardCategory.RED_FLAG, severity=Severity.HALT,
    ...
    for i, (pat, desc) in enumerate(_DEMEDICALIZE_PATTERNS):
        rules.append(GuardRule(
            id=f"DM-{i+1:03d}", category=GuardCategory.DEMEDICALIZE, severity=Severity.BLOCK,
            description=desc,
    ...
    for i, (pat, desc) in enumerate(_AXIS_REDLINE_PATTERNS):
        rules.append(GuardRule(
            id=f"AX-{i+1:03d}", category=GuardCategory.AXIS_REDLINE, severity=Severity.BLOCK,
            description=desc,
            matcher=lambda t, p=pat: re.search(p, t, re.IGNORECASE) is not None,
...
def pre_gate(user_input: str) -> GateResult:
    ...
    for rule in RULES:
        if rule.category != GuardCategory.RED_FLAG:
            continue
        hit = rule.check(user_input)
        if hit:
            snippet = _redflag_find(user_input) or user_input
            findings.append(Finding(rule.id, rule.category, rule.severity,
                                    snippet, rule.message, rule.suggestion))
    if findings:
        return GateResult(passed=False, level="halt", findings=findings)
    return GateResult(passed=True, level="none", findings=[])


def post_gate(generated: str, cited_evidence: Optional[List[str]] = None) -> GateResult:
    ...
    if cited_evidence is not None and len(cited_evidence) == 0 and "依据" in generated:
        findings.append(Finding(
            "EG-001", GuardCategory.EVIDENCE_GRADE, Severity.WARN,
            generated[:80], "声明有依据但引用为空，证据链断裂。",
            "补充古籍条目/基因位点/案例 ID 之一作为溯源。",
        ))
    level = "halt" if any(f.severity == Severity.HALT for f in findings) else \
            ("caution" if findings else "none")
...
def _demo():
    print("=== agent_safety_gate 演示（借鉴 MedAssist 前置红牌 + LabGuard typed guard IR）===\n")
    ...
    cases = [
        ...
        ("生成：从A轴（气化）看，建议早睡并晨间光照，依据见古籍《黄帝内经》", "post", ["黄帝内经/素问"]),
    ]
    ...
    for text, stage, cited in cases:
        if stage == "pre":
            ...
            r = post_gate(text, cited)
            tag = "生成后闸门"
        print(f"[{tag}] {text}")
        print(f"  -> passed={r.passed} level={r.level} unsafe_event={r.unsafe_event}")
        for f in r.findings:
            print(f"     ! {f.rule_id}/{f.category.value}/{f.severity.value}: {f.message}")
        print()

    # 量化：统计一组样本的不安全事件率（LabGuard 式指标）
    sample = [
        pre_gate("我胸痛"), post_gate("给你开药方", []),
        post_gate("建议早睡", ["黄帝内经"]), pre_gate("最近疲劳"),
    ]
    unsafe = sum(1 for s in sample if s.unsafe_event)
    print(f"不安全事件率（演示样本）: {unsafe}/{len(sample)} = {unsafe/len(sample):.0%}")
